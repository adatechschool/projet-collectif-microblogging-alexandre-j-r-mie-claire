...
class="mt-6">
    <div class="max-w-4xl px-10 py-6 mx-auto bg-white rounded-lg shadow-md">
        <div class="flex items-center justify-between"><span class="font-light text-gray-600">{{ $post->created_at->format('F j, Y') }}</span>
        </div>
        <div class="mt-2">
                 ...
                 <div class="flex items-center justify-between text-gray-500">
                    <div class="flex items-center space-x-2">
                        <form method="POST" action="{{ route('like',['post' => $post]) }}" >
                            @csrf
                        <button type="submit" class="flex justify-center items-center gap-2 px-2 hover:bg-gray-50 rounded-full p-1">
                    ...
                    <button class="flex justify-center items-center gap-2 px-2 hover:bg-gray-50 rounded-full p-1">
                        ...
                        <span><a href={{"/post/".$post->id}} class="flex items-center">{{$post->comment()->count()}} Comment</a></span>
